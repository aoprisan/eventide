...
import { computeInsights, type ModuleId } from '@eventide/engine';
      ...
      {!hasData ? (
        ...
        <>
          <div className="stat-grid stagger">
            <Stat num={insights.currentStreak} label="Day streak" />
            <Stat num={insights.totalSessions} label="Sessions" />
            <Stat num={insights.totalMinutes} label="Minutes" unit="min" />
            <Stat num={insights.longestStreak} label="Longest streak" />
          </div>
            ...
            <div className="col gap-sm" style={{ marginTop: 26 }}>
              ...
              {moduleRows.map((r) => (
                <div key={r.id} className="list-row card">
                  <span className="grow" style={{ fontFamily: 'var(--font-display)', fontSize: '1.2rem' }}>
                    {MODULES[r.id as 'breath' | 'candle' | 'meditation'].name}
                  </span>
                  <span className="muted">{r.sessions} · {Math.round(r.minutes)} min</span>
                </div>
              ))}
            </div>
          )}
...
function Stat({ num, label, unit }: { num: number; label: string; unit?: string }) {
  return (
